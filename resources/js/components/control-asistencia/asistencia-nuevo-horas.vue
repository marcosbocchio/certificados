    ...
    methods: {
        ...
        async confirmar() {
            ...
            if (!this.frente_selected) {
                ...
                this.isDis = false;
                this.isLoading = false; // Detener carga si hay error
                return;
            }
            if (!this.fecha) {
            ...
            for (let detalle of this.detalles) {
                ...
                if (detalle.parte && detalle.parte.length > 1) {
                    ...
                    if (!parteEsValido) {
                        ...
                        this.isLoading = false;
                        this.isDis = false; // Re-habilita el botón en este error también
                        return;
                    }
                }
            }
            ...
            axios.post('/api/guardar_asistencia', {
                ...
                .then(response => {
                    ...
                    // El botón queda deshabilitado y la página redirige, lo cual es perfecto.
                    window.location.href = '/area/enod/asistencia/horas';
                })
                .catch(error => {
                    toastr.error('Error al guardar:', error);
                    // Se recomienda re-habilitar el botón si la API falla, para que el usuario pueda reintentar.
                    this.isDis = false;
                })
                .finally(() => {
                    // Solo es necesario detener el estado de carga.
                    this.isLoading = false;
                });
        },
