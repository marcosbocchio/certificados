                            ...
                            <td class="text-center">
                                ...
                                <label v-if="mostrarSDFCheckboxCol">
                                    <input type="checkbox" v-model="detalle.s_d_f_sn"
                                        :disabled="detalle.contratista != null">
                                </label>
...
export default {
    ...
    methods: {
        ...
        async confirmar() {
            ...
            if (!todosValidos) {
                // 3. Si algo es inválido, detener la carga y RE-HABILITAR el botón.
                this.isLoading = false;
                this.isDis = false;
                return; // Detenemos la ejecución
            }

            // Si todo es válido, procedemos con el envío a la API.
            axios.post(`/api/asistencia/${this.asistenciaId}/update`, {
                ...
                // El botón se queda deshabilitado mientras la página redirige. ¡Perfecto!
                window.location.href = '/area/enod/asistencia/horas';
            }).catch(error => {
                toastr.error('Error al actualizar los detalles');
                // 4. Si la API falla, RE-HABILITAR el botón para que el usuario pueda reintentar.
                this.isDis = false;
            }).finally(() => {
                // 5. El loader se desactiva siempre, tanto en éxito como en error.
                this.isLoading = false;
            });
        },
