...
export default {
  ...
  methods: {
    agregarNuevo() {
      window.location.href = '/area/enod/asistencia-nuevo-horas';
    },
    editar(item) {
      window.location.href = `/asistencia/edit/horas/${item.id}`;
    },
    copiar(item) {
    ...
    fetchData(page = 1) {
      ...
      axios.get('/api/area/enod/asistencia/horas', { params })
        .then(response => {
