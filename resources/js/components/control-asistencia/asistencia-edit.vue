<template>
    <div>
            ...
            <div class="box-body row">
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <v-select v-model="frente_selected" :options="frentes_opciones" label="codigo" id="frente"
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <date-picker id="fecha" v-model="fecha" value-type="YYYY-MM-DD" format="DD-MM-YYYY"
            ...
            <div class="box-body row">
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <v-select v-model="operador_selected" :options="operarios_opciones" label="name"
                            id="operador"></v-select>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="operador">ayudante</label>
                        <v-select v-model="operador_ayudante" :options="operarios_opciones" label="name"
                            id="ayudante"></v-select>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <v-select v-model="contratista_selected" :options="contratistas_opciones"
                            label="nombre_fantasia" id="contratista"></v-select>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                ...
                <table class="table table-hover table-striped table-condensed">
                    ...
                    <tbody>
                        <tr v-for="(detalle, index) in detalles" :key="index">
                            ...
                            <td class="text-center" v-if="detalle.ayudante && detalle.ayudante.name">{{
                                detalle.ayudante.name }}
                            </td>
                            <td class="text-center" v-else>-</td>
                            <td>
                                <input type="time" v-model="detalle.entrada" class="form-control"
                                    @change="calcularHorasExtrasDetall(detalle, index)" />
                            </td>
                            <td>
                            ...
                            <td>
                                <v-select v-model="detalle.contratista" :options="contratistas_opciones"
                                    label="nombre_fantasia" :reduce="contratista => contratista"></v-select>
                            </td>
                            <td>
                                <input type="text" v-model="detalle.parte" class="form-control" />
                            </td>
                            <td class="text-center">
                                <!-- Si detalle.metodo_ensayo existe, muestra el v-select -->
                                <v-select v-if="detalle.metodo_ensayo" v-model="detalle.metodo_ensayo"
                                    :options="metodo_ensayos" label="metodo" :reduce="option => option">
                                ...
                                <v-select v-else v-model="detalle.metodo_ensayo" :options="metodo_ensayos"
                                    label="metodo" :reduce="option => option">
                                    <!-- Template para customizar la opción en el dropdown -->
                                    <template #option="option">
                                        <span class="upSelect">{{ option.metodo }}</span><br>
                                        <span class="downSelect">{{ option.descripcion }}</span>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content custom-modal">
                    ...
                    <div class="modal-body">
                        <textarea v-model="observacionTexto" class="form-control custom-textarea" rows="4"
        ...
        <div class="form-actions">
            <div class="col-md-12">
                <button @click="confirmar" class="btn btn-enod" :disabled="isDis">Guardar</button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async confirmar() {
            // 1. Deshabilitar el botón y activar el loader inmediatamente.
            this.isDis = true;
            this.isLoading = true;

            // 2. Verificar todos los partes antes de guardar.
            const todosValidos = await this.verificarTodosLosPartes();
            if (!todosValidos) {
                // 3. Si hay un error de validación, detenemos el loader y RE-HABILITAMOS el botón.
            ...
            axios.post(`/api/asistencia/${this.asistenciaId}/update`, {
                ...
                // El botón se mantiene deshabilitado mientras la página redirige, lo cual es correcto.
                window.location.href = '/area/enod/asistencia/servicios';
            }).catch(error => {
                toastr.error('Error al actualizar los detalles');
                // 4. Si la API falla, RE-HABILITAMOS el botón para un nuevo intento.
                this.isDis = false;
            }).finally(() => {
                // 5. El loader se desactiva siempre al final, ya sea con éxito o error.
                this.isLoading = false;
            });
        },
