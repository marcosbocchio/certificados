...
methods : {
    ...
    async downloadPdf_tab1 (){
        ...
        doc.addImage(imgData,'PNG',60,70,90,90);

        doc.setFontSize(11);
        doc.text("Análisis de rechazos por Diámetro", 15, 166)
       ...
       doc.text("Análisis de rechazos por Espesor", 15,doc.lastAutoTable.finalY + 14)
        doc.autoTable({
            ...
            for(let i = 0; i < pageCount; i++) {
                ...
                doc.line(11, 28, 200, 28);

                /* header entre lineas */
                doc.setFontSize(10);
                doc.setFontType("bold");
                doc.text("Cliente", 14, 34);
                doc.text("Obra", 150, 34);
                doc.text("Desde", 170, 34);

                doc.setLineWidth(0.3);
                doc.setDrawColor(32,178,170);
                doc.line(11, 42, 200, 42);

                doc.text("Proyecto", 14, 48);
                doc.text("OT Nº", 150, 48);
                doc.text("Hasta", 170, 48);

                /* Datos del header*/
                doc.setFontType("normal");
                doc.setFontType("italic");

                doc.text(this.cliente.nombre_fantasia, 14, 39)
                doc.text(this.obra.obra, 150, 39)
                doc.text((this.fecha_desde ? moment( this.fecha_desde).format("DD/MM/YYYY") : ' '), 170, 39)

                doc.text(this.ot.proyecto, 14, 53)
                doc.text(this.ot.numero.toString(), 150, 53)
                doc.text((this.fecha_hasta ? moment( this.fecha_hasta).format("DD/MM/YYYY") : ' '), 170, 53)

                /* linea amarilla */

                doc.setLineWidth(0.8);
                doc.setDrawColor(255,204,0);
                doc.line(11, 60, 200, 60);
