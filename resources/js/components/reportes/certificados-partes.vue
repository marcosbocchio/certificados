        ...
        <div class="col-md-9">
            ...
            <tabs :options="{ useUrlFragment: false }">
                <tab name="Certificados">
                    <div v-if="tablaCertificados.length > 0">
                        <div class="row">
                          <div class="col-lg-12">
                            <table class="table table-hover table-striped table-bordered table-condensed">
                              ...
                              <tbody>
                                  <tr v-for="(item,k) in tablaCertificados" :key="k">
                                      <td> {{ item.fecha}}</td>
                 ...
                 <tab name="Partes">
                    <div v-if="tablaPartes.length > 0">
                        <div class="row">
                          <strong class="checkbox-right" ><input type="checkbox" v-model="filtrado" > Sin certificado</strong>
                          <div class="col-lg-12">
                            <table class="table table-hover table-striped table-bordered table-condensed">
                              <thead>
                                  <tr>
                              ...
                              <tbody>
                                  <tr v-for="(item,k) in partes_filter" :key="k">
                                      <td> {{ item.fecha}}</td>
                                      <td style="text-align:center">
...
export default {
    ...
    computed :{
        ...
        return (this.filtrado === true) ? this.tablaPartes.filter(e => !e.certificado) : this.tablaPartes;
        },
    },
...
methods :{
    ...
    async Buscar() {
     ...
     this.total_placas = 0;
        ...
        try {
            let url4 = 'reporte-partes' + '/cliente/' + (this.cliente ? this.cliente.id : 'null')  + '/ot/' + (this.ot ? this.ot.id : 'null' )  + '/obra/' + (this.obra ? this.obra.obra.replace('/','--') : 'null' ) + '/fecha_desde/' + this.fecha_desde + '/fecha_hasta/' + this.fecha_hasta +'?api_token=' + Laravel.user.api_token;
            let url3 = 'reporte-certificados' + '/cliente/' + (this.cliente ? this.cliente.id : 'null')  + '/ot/' + (this.ot ? this.ot.id : 'null' )  + '/obra/' + (this.obra ? this.obra.obra.replace('/','--') : 'null' ) + '/fecha_desde/' + this.fecha_desde + '/fecha_hasta/' + this.fecha_hasta +'?api_token=' + Laravel.user.api_token;
