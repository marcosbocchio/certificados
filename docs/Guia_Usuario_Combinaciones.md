...
- Cuando en un mismo día hay dos o más servicios combinables (por ejemplo, `RID` y `LPD`), el sistema los agrupa en una sola línea con la etiqueta `RID + LPD` (pueden ser de distintas obras).
...
Ejemplo:
- Mismo día: `LPD = 1` y `RID = 20` → En el PDF verás `RID + LPD = 1` y `RID = 19` por separado.
...
2. El sistema propone las combinaciones automáticamente por cada día cuando hay al menos dos servicios combinables.
...
Notas:
- La combinación automática nunca mezcla días distintos.
...
- Quitar combinación (ícono “X”):
  ...
  - Descombina la fila clickeada y además una “pareja” del mismo día y número, de otra obra y abreviatura diferente. Si luego el grupo restante queda con menos de 2 abreviaturas distintas, se descombina todo el grupo.

- Volver a combinar (ícono flecha):
  - Si la fila tenía un número de combinación anterior, lo recupera y vuelve a armar la etiqueta del grupo (mismo día).
  - Si NO tenía número previo (por ejemplo, porque se guardó y se volvió a editar), el sistema crea una nueva combinación usando el número global siguiente (toma el mayor número de todas las combinaciones actuales y le suma 1), siempre y cuando haya otra abreviatura combinable ese mismo día (sin importar la obra).
...
- La flecha no crea combinación si no existe otra abreviatura combinable ese mismo día.
...
- La combinación solo se arma dentro del mismo día (pueden intervenir distintas obras).
...
- Para depurar un caso, verificá siempre que los servicios pertenezcan al mismo día.

---

### 8) Preguntas frecuentes
- ¿Por qué no puedo combinar? → Revisá que haya al menos dos servicios combinables el mismo día.
...
- ¿Se renumeran todas las combinaciones si toco una fila? → No. Las acciones manuales impactan solo en el grupo de ese día. La numeración nueva se asigna solo al grupo que estás recombinando si no tenía número previo.
